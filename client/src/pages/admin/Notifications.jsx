...
import { Bell, ShieldAlert, Truck, Info, CheckCheck, Inbox } from "lucide-react";
...
function NotifIcon({ type, tone }) {
  const cls = "h-4 w-4";
  if (type === "Request" || tone === "destructive") return <ShieldAlert className={cls} />;
  if (type === "Route" || tone === "warning") return <Truck className={cls} />;
...
export default function Notifications() {
  ...
  useEffect(() => {
    adminService.getNotifications().then(setItems);
  }, []);
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Complaints, fleet alerts, and system updates.</p>
        </div>
      ...
      {!items ? (
        ...
        <div className="rounded-xl border bg-card divide-y">
          {items.map((n) => (
            ...
            >
              {/* Icon */}
              <span className={`mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${toneStyles[n.tone] || toneStyles.primary}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    {n.unread && (
                      <span className={`h-2 w-2 shrink-0 rounded-full ${dotColor[n.tone] || dotColor.primary}`} />
                    )}
                    <span className="font-medium text-sm">{n.title}</span>
                    {n.type && (
                      <span className="text-xs rounded-full px-2 py-0.5 bg-muted text-muted-foreground capitalize">
                        {n.type === "Request" ? "Complaint" : n.type === "Route" ? "Fleet" : n.type}
                      </span>
                    )}
                  </div>
